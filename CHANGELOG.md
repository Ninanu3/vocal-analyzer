...
#### 신규 기능
- **갈라짐(성구 전환) 탐지** — voiced_prob 기반 단기 비발성 구간(0.05~0.5s) 탐지
  - 타임스탬프 + 음역대 아이콘(🔵중음/🟡중음/🔴고음)으로 표시
- **비브라토 분석** — held note 구간 F0에 FFT 적용, 4~8Hz 피크 탐지
  - 속도(rate), 폭(extent), 커버리지(%), 상태 레이블 출력
- **호흡 패턴 분석** — voiced gap 0.25~2.0s를 breath cycle로 집계
  - 평균 프레이즈 길이, 중간 호흡 경고, 호흡 간격 분포 분석
- **발성 피로도 감지** — 전반부 vs 후반부 Jitter/HNR 비교
  - 피로 점수 계산, 개선/유지/악화 판정
- **자연어 코칭 피드백** — 수치 기반 구체적 훈련 권고 (SOVT, 메사 디 보체, 립 트릴 등)
- **`/report` 명령어** — 최근 20세션 추이 분석 + 트렌드 차트 전송
  - 음역대별 안정성 지도 포함

#### 기술 변경
- `_run_pyin()` voiced_prob 세 번째 반환값 적극 활용
- 각 신규 분석 함수 독립 try/except로 격리 — 메인 결과 영향 없음
- `generate_trend_chart()` 추가 (Jitter/HNR/F2 꺾은선 차트)
...
#### 신규 기능
- **구간별 문제 탐지** — 어느 시간대, 어느 음높이에서 문제가 생기는지 표시
  - `⚠️ 0:12  📳 Jitter 1.72% — E4 부근` 형태로 타임스탬프와 함께 출력
  - 성구 전환(보컬 브레이크): F0 비율 1.4배 이상 급변 지점 탐지 `🎭`
  - 비음 경향 구간: 200~500Hz 비강 공명 에너지 비율이 높은 구간 근사 탐지 `👃`
  - Jitter 1.5% 이상 held note 구간 → 경고 `📳`
- **음역대별 분석** — 저음/중음/고음 구간 각각의 Jitter/HNR 비교
  - 음이름 범위 표시 (예: A3~C4, 3구간)
  - 고음에서만 떨림이 심한 패턴 등 파악 가능
- `_hz_to_note()` 추가 — 주파수(Hz) → 음이름(예: 330Hz → E4) 변환

#### 기술 변경
- `_detect_held_notes` 분리 → `_run_pyin` + `_extract_held_segments` (f0 데이터 공용 사용)
- 추가 분석(성구 전환/비음/음역대 통계)은 독립된 try/except로 격리 — main 결과 영향 없음
